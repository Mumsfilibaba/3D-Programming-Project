...
namespace Math
{
	...
	Vector2 Matrix2::Multiply(const Vector2& vector) const
	{
		...
		t[0] = (rows[0].v[0] * vector.v[0]) + (rows[0].v[1] * vector.v[1]);
		t[1] = (rows[1].v[0] * vector.v[0]) + (rows[1].v[1] * vector.v[1]);
	...
	float& Matrix2::GetElement(uint8 r, uint8 c)
	{
		assert(r < 2);
		assert(c < 2);
	...
	float Matrix2::GetElement(uint8 r, uint8 c) const
	{
		assert(r < 2);
		assert(c < 2);
	...
	Vector2& Matrix2::GetRow(uint8 r)
	{
		assert(r < 2);
	...
	Vector2 Matrix2::GetRow(uint8 r) const
	{
		assert(r < 2);
	...
	float Matrix2::operator()(uint8 r, uint8 c) const
	{
		return GetElement(r, c);
	}

	float& Matrix2::operator()(uint8 r, uint8 c)
	{
		return GetElement(r, c);
	}

	Vector2 Matrix2::operator()(uint8 r) const
	{
		return GetRow(r);
	}

	Vector2& Matrix2::operator()(uint8 r)
	{
		return GetRow(r);
	}
